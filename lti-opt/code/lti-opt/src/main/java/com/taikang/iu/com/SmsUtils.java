...
public class SmsUtils {
    ...
    public SendSmsResponse sendSms(String phoneNumber,String signName,String templateCode,
    	...
    	log.info("<======开始发送短信通知，手机号：{}，签名：{}，模板：{}，时间：{}===============>",phoneNumber,signName,templateCode,sdf.format(new Date()));
        //可自助调整超时时间
        ...
        try {
        	...
        	if(!"OK".equals(sendSmsResponse.getCode())){
        		log.error("<======发送短信错误，错误码：{}===============>",sendSmsResponse.getCode());
	       	}
		} catch (Exception e) {
			log.error("<======发送短信错误，手机号：{}，签名：{}，模板：{}，错误信息：{}===============>",phoneNumber,signName,templateCode,e);
		}

        log.info("<======结束发送短信通知，时间：{}===============>",sdf.format(new Date()));
