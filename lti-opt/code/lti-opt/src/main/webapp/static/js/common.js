...
$.extend($.fn.validatebox.defaults.rules, {
			...
			mobile : {// 验证手机号码
				validator : function(value) {
					return /^1[3|4|5|7|8][0-9]\d{8}$/i.test(value);
				},
				message : '手机号码格式不正确'
			},
			mail:{
				validator : function(value) {//验证邮箱格式
					return /^[A-Za-zd]+([-_.][A-Za-zd]+)*@([A-Za-zd]+[-.])+[A-Za-zd]{2,5}$/i.test(value);
				},
				message : '邮箱格式不正确'
			},
			...
			username : {// 验证用户名
				...
				message : '用户名不合法（字母或数字开头，允许6-16字节，允许字母数字下划线）'
			},
			faxno : {// 验证传真
				validator : function(value) {
					return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value);
