		...
		<form id="dataForm1" method="post">
			 <table style="width:100%; height:1%; overflow: hidden;" border="0" 
				...
				<tr>
					...
					<td style="height:10px;" class="kTableLabel lbl">订单状态:</td>
					<td style="height:10px;"><select class="easyui-combobox" name="orderStatus" id="orderStatus" style="width:80px" panelHeight="100px">
									<option value="5" selected>已退单</option>
...
jQuery(function(){	  
		...
		columns:[[
	          			// 注意：这里是字段名SELLER_CODE，不是变量名sellerCode
							...
							{field:'screen_name',title:'屏幕',width:60},
							{
								...
								width : 60,
								formatter : function(value, row,index) {
									return "<a href='#' onclick=confirmrow('"+row.order_id+"','"+row.order_type+"') style='margin-left:20px'>[退款]</a>"
								}
							}
						//注：最后一行后面的逗号要去掉
		]],
	...
	function searchInfo() {
		...
		var url = '${rootPath}/orderRefund/list';
...
// 		$('#confirmStatus').combobox('setValue','0');
   ...
   function confirmrow(rowId,orderType){    	  
       if (rowId){
    	   	url = '${rootPath}/orderRefund/refundView?rowId='+ rowId + '&orderType=' + orderType;
	   		$("#divDialog").dialog({
	   			title : "订单退款",
	   			width : 600,
	   			height : 580,
